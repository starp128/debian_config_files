...
static void modeset_draw2(void);
static void modeset_draw_cairo(void);
			     ...
			     struct modeset_dev *dev)
{
	...
	fprintf(stderr, "select mode index %d\n", mode_index);
	/* copy the mode information into our device structure */
	memcpy(&dev->mode, &conn->modes[mode_index], sizeof(dev->mode));
	dev->width = conn->modes[mode_index].hdisplay;
	dev->height = conn->modes[mode_index].vdisplay;
...
static void modeset_draw2(void)
{
	...
	b = 0x0;
	for (iter = modeset_list; iter; iter = iter->next) {
		for (j = 100; j < iter->height - 100; ++j) {
			for (k = 100; k < iter->width - 100; ++k) {
				off = iter->stride * j + k * 4;
				*(uint32_t*)&iter->map[off] =
					(r << 16) | (g << 8) | b;
			}
	...
	b = 0xff;
	for (iter = modeset_list; iter; iter = iter->next) {
		for (j = 200; j < iter->height - 200; ++j) {
			for (k = 200; k < iter->width - 200; ++k) {
				off = iter->stride * j + k * 4;
				*(uint32_t*)&iter->map[off] =
					(r << 16) | (g << 8) | b;
			}
...
static void modeset_draw_cairo(void)
{
	...
	line_length = iter->stride;
	surface = cairo_image_surface_create_for_data (iter->map, CAIRO_FORMAT_ARGB32, 
